---
name: git-commit
allowed-tools: Bash(gh auth status), Bash(gh auth switch:*), Bash(git remote get-url:*), Bash(git branch:*), Bash(git status:*), Bash(git log:*), Bash(git diff:*), Bash(git add:*), Bash(git commit:*), Bash(gh repo:*), Bash(bash:*)
...
- Git のアクティブアカウントとリポジトリ所有者: !`bash ${CLAUDE_SKILL_DIR}/scripts/check-account.sh`
- 現在のブランチ: !`git branch --show-current`
- 作業ツリーの状態（追跡外ファイル含む）: !`git status --short`
- 変更ファイルの概要: !`git diff --stat HEAD 2>/dev/null || echo "(コミット履歴なし)"`
- ステージ済み差分: !`git diff --staged`
- 未ステージ差分: !`git diff`
- 直近コミット（メッセージ様式の参考）: !`git log --oneline -15 2>/dev/null || echo "(コミット履歴なし)"`
...
1. Git のアクティブアカウントとリポジトリの所有者を確認し、もし異なるようであればアクティブアカウントをリポジトリの所有者に変更する
   ...
   - 所有者が「未解決」と表示された場合（リポジトリが GitHub 上に未作成など）は、アカウント切替を行わずそのまま続行する。コミットはローカル操作のため GitHub 解決は不要。
