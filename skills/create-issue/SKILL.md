...
allowed-tools: Bash(gh auth status), Bash(gh auth switch:*), Bash(git remote:*), Bash(git branch:*), Bash(gh repo view:*), Bash(gh issue create:*), Bash(gh issue list:*), Bash(gh label list:*)
---
...
- リポジトリの所有者: !`gh repo view --json owner -q .owner.login`
...
1. アクティブアカウントとリポジトリ所有者を照合し、異なる場合はアクティブアカウントを所有者に切り替える
   - 切替コマンド: `gh auth switch --user [username]`
   - これを怠ると `gh` が別アカウントで叩かれ、起票先を取り違えたり権限エラーで失敗するため最初に揃える

2. 起票先リポジトリ(nameWithOwner)をユーザーに明示し、ここで合っているか確認する
